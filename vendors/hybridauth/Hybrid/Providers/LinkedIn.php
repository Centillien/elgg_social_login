	...
	*/
	function getUserProfile()
	{
		try{
			...
			$response = $this->api->profile('~:(id,first-name,last-name,public-profile-url,headline,primary-twitter-account,picture-url,date-of-birth,phone-numbers,location,email-address,summary)');
		}
		...
		if( isset( $response['success'] ) && $response['success'] === TRUE ){
			...
			$this->user->profile->skills     =  @ (string) $data->{'specialties'};

			$this->user->profile->bday     =  @ (string) $data->{'date-of-birth'};
			$this->user->profile->twitter     =  @ (string) $data->{'primary-twitter-account'};
			...
			$this->user->profile->briefdescription = @ (string) $data->{'headline'};
